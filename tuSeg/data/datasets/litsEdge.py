...
class LiTSEdgeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        edges = LabelToContour(kernel_type="Laplace")(labels)
        ...
        edges = torch.from_numpy(np.transpose(np.array(edges), (3, 0, 1, 2))).type(torch.float32)

        if self.one_hot:
            labels = one_hot_encode(labels, num_classes=3, dim=0)

        return {'image': image, 'labels': labels, 'edges': edges}

if __name__ == '__main__':
    test = LiTSEdgeDataset('/data1/lsx/data/LiTS')